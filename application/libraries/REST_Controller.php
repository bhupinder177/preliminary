...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function _detect_api_key()
    {
        ...
        //echo $api_key_variable; die;
        // Work out the name of the SERVER entry based on config
        $key_name = 'HTTP_' . strtoupper(str_replace('-', '_', $api_key_variable));
         $user_name = 'HTTP_' . strtoupper(str_replace('-', '_', $this->config->item('rest_userid_column')));
        ...
        //echo $this->input->server($key_name); die;
        // Find the key from server or arguments


        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
             $userId = isset($this->_args[$this->config->item('rest_userid_column')]) ? $this->_args[$this->config->item('rest_userid_column')] : $this->input->server($user_name);

            if (!($row = $this->rest->db->where($this->config->item('rest_key_column'), $key)->where($this->config->item('rest_userid_column'), $userId)->get($this->config->item('rest_keys_table'))->row()))
            {
               return FALSE;
                   }
            ...
            $this->_apiuser = $row;
            /**
             * [Creating session variable at the start of request and destroy at the end of that request(Service client)]
             */
 ////////////////////no need now/////////
  // $userDataGlobal = $this->_get_userdata($this->_apiuser->Email, $this->_apiuser->Password);
           // $this->_get_session($userDataGlobal['data']);
...
////////////////////no need now/////////
    ...
    protected function _log_request($authorized = FALSE)
    {
        ...
        $is_inserted = $this->rest->db
            ->insert(
                ...
                // 'params' => $this->_args ? implode($this->_args) : NULL,
                // 'params' => $this->input->raw_input_stream,
                'api_key' => isset($this->rest->key) ? $this->rest->key : '',
                'userId' => isset($this->_apiuser->UserId) ? $this->_apiuser->UserId : '',
    ...
    /**
     * Updates the log table with HTTP response data
     *
     * @access protected
     * @author M4n07
     * @param $data array HTTP response data
     * @return bool TRUE log table updated; otherwise, FALSE
     */
    protected function _log_response_data($data)
    {
        $payload['paramsresponse'] = json_encode($data);
    ...
    /**
     * [Insert a key into the database for new user]
     * @param  [array]  $userData [User data]
     * @param  [String] $password [Password]
     * @return [String] $key      [Api Key]
     */
    protected function createKey($userid,$user_ref) //by ridhima
    {
        // Build a new key
        $key = $this->_generate_key();
        //$deviceKey = $this->_generate_key();
        // Insert the new key
        if ($this->_insert_key($key,$userid))
        {
          $this->db->set('user_reference',$user_ref);
          $this->db->where('user_id',$userid);
          $this->db->update('hc_keys');
          return $key;
        }
        else
        {
    ...
    /**
     * [Regenerate a key for existing user]
     * @param  [int]    $userId  [User Id]
     * @return [String] $new_key [new Api Key]
     */
    protected function regenerateKey($userId, $deviceId,$registerId)
    {
        // Build a new key
        $new_key = $this->_generate_key();

        // Update the new key
        if ($this->_update_key($userId ,$new_key, $deviceId,$registerId))
        {
            return  $new_key;
        }
        else
     ...
     */
    protected function _get_userdata($username, $password)
    {
        $this->load->model('login');
        return $this->login->clientApiLogin($username,$password);
    }
    /**
     * [Get companies list for current user]
     * @param  [String] $clientId
     * fields to fetch [CID,Name]
     * @return [array] [companies list]
     */
    protected function _getAllBooksData()
    {
        $this->load->model('login');
        return $this->login->getAllBooksData();
    }


    /**
     * [Create user session for request servicing]
     * @param  [array] $userData [User Data]
     * @return [User session set]
     */
    protected function _get_session($userData)
    {
    ...
    /**
     * [Destroy user session set for request servicing]
     * @return  [Destroy user session]
     */
    ...
    /**
     * [Get user data from Api key Table]
     * @param  [int]   $userId [description]
     * @return [array]         [Api key data]
     */
    protected function _get_key_user($userId,$DeviceId,$registerId)
    {
        return $this->db
            ->where('userId', $userId)
    ...
    /**
     * [Get apikey data from Api key Table]
    ...
    /**
     * [Check for existence of key in table]
     * @param  [String] $key [Api Key]
     * @return [Boolean]     [If key exist true, Else False]
     */
    ...
    /**
     * [Insert Data in api key Table]
     * @param  [String] $key      [Api key]
     * @param  [array]  $userData [User Data]
     * @param  [String] $password [Password]
     * @return [Insert data in api key Table]
     */
    protected function _insert_key($key,$userid) //by raman
    {
        $data = [];
        $data['security_key'] = $key;
        $data['user_id'] = $userid;

       return $this->db->insert('hc_keys',$data);
    }

    /**
     * [Update apikey in api key Table]
     * @param  [int] $userId [User Id]
     * @param  [String] $newKey [New Api key]
     * @return [Update new key in api key Table]
     */
    protected function _update_key($userId, $newKey=NULL, $deviceId,$registerId)//by raman
    {
        return $this->db
            ->set(config_item('rest_key_column'),$newKey)
            ->set('registeredId ',$registerId)
            ->where('deviceId',$deviceId)
            ->where('userId', $userId)
          //  ->where('registerID', $registerId)
            ->update(config_item('rest_keys_table'));
    }
    protected function _delete_key($userId,$deviceId) //by raman
    {
         $this->db->where('deviceId',$deviceId);
         $this->db->where('userId',$userId);
         $this->db->delete(config_item('rest_keys_table'));
         return true;

    }
    protected function  checkSecurityKey($key,$userId) //by raman
    {
     $this->db->select('security_key,user_id');
     $this->db->from('hc_keys');
